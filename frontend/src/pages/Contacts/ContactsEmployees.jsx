...
const DeptModal = ({ dept, users, onClose, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      if (dept) { await api.put(`/departments/${dept.id}`, form); toast.success('تم تحديث الإدارة'); }
      else { await api.post('/departments', form); toast.success('تم إضافة الإدارة'); }
      onSave();
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.55)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
      <div style={{ background: 'white', borderRadius: '20px', width: '100%', maxWidth: '440px', boxShadow: '0 25px 50px rgba(0,0,0,0.2)', overflow: 'hidden' }}>
        <div style={{ padding: '18px 22px', background: 'linear-gradient(135deg, #0f172a, #1e293b)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h3 style={{ margin: 0, color: 'white', fontWeight: 800 }}>{dept ? 'تعديل الإدارة' : 'إضافة إدارة'}</h3>
        ...
        <form onSubmit={handleSubmit} style={{ padding: '22px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <div>
            ...
            <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="مثال: قسم المبيعات" style={inputStyle} required />
          </div>
          <div>
          ...
          <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
            <button type="button" onClick={onClose} style={{ padding: '10px 18px', background: '#f1f5f9', border: 'none', borderRadius: '10px', fontWeight: 700, cursor: 'pointer', color: '#64748b' }}>إلغاء</button>
...
const JobTitleModal = ({ jt, onClose, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!name.trim()) return toast.error('اسم الوظيفة مطلوب');
    setSaving(true);
    try {
      if (jt) { await api.put(`/job-titles/${jt.id}`, { name }); toast.success('تم التحديث'); }
      else { await api.post('/job-titles', { name }); toast.success('تم إضافة الوظيفة'); }
      onSave();
    } catch (err) { toast.error(err.response?.data?.message || 'حدث خطأ'); }
    finally { setSaving(false); }
  };
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.55)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
      <div style={{ background: 'white', borderRadius: '20px', width: '100%', maxWidth: '380px', boxShadow: '0 25px 50px rgba(0,0,0,0.2)', overflow: 'hidden' }}>
        <div style={{ padding: '18px 22px', background: 'linear-gradient(135deg, #7c3aed, #a855f7)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h3 style={{ margin: 0, color: 'white', fontWeight: 800 }}>{jt ? 'تعديل الوظيفة' : 'إضافة وظيفة'}</h3>
          <button onClick={onClose} style={{ background: 'rgba(255,255,255,0.1)', border: 'none', color: 'white', borderRadius: '8px', padding: '6px 10px', cursor: 'pointer' }}><X size={16} /></button>
        </div>
        <form onSubmit={handleSubmit} style={{ padding: '22px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <div>
            <label style={labelStyle}>اسم الوظيفة *</label>
            <input value={name} onChange={e => setName(e.target.value)} placeholder="مثال: مدير مبيعات" style={inputStyle} required />
          </div>
          <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
            <button type="button" onClick={onClose} style={{ padding: '10px 18px', background: '#f1f5f9', border: 'none', borderRadius: '10px', fontWeight: 700, cursor: 'pointer', color: '#64748b' }}>إلغاء</button>
...
const EmployeeModal = ({ emp, departments, jobTitles, onClose, onSave }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!emp && !form.email.trim()) return toast.error('البريد الإلكتروني مطلوب');
    if (!emp && !form.password) return toast.error('كلمة المرور مطلوبة');
    setSaving(true);
    try {
      if (emp) {
  ...
  const tabs = [
    { id: 'info', label: 'البيانات الأساسية', icon: <User size={14} /> },
    { id: 'attachments', label: 'المرفقات', icon: <Paperclip size={14} />, disabled: !emp },
  ];
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(15,23,42,0.6)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '16px' }}>
      <div style={{ background: 'white', borderRadius: '20px', width: '100%', maxWidth: '680px', maxHeight: '92vh', overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 25px 60px rgba(0,0,0,0.25)' }}>
        ...
        <div style={{ padding: '18px 24px', background: 'linear-gradient(135deg, #4f46e5, #06b6d4)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h3 style={{ margin: 0, color: 'white', fontWeight: 800 }}>{emp ? 'تعديل بيانات الموظف' : 'إضافة موظف جديد'}</h3>
        ...
        <div style={{ display: 'flex', gap: '4px', padding: '12px 20px 0', borderBottom: '1px solid #e2e8f0' }}>
          {tabs.map(t => (
            <button key={t.id} onClick={() => !t.disabled && setActiveTab(t.id)} disabled={t.disabled}
              ...
              {t.icon} {t.label} {t.disabled && <span style={{ fontSize: '10px' }}>(حفظ أولاً)</span>}
            </button>
          ))}
        </div>
          ...
          {activeTab === 'info' ? (
            <form id="emp-form" onSubmit={handleSubmit}>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
                <div style={{ gridColumn: '1/-1' }}>
                  ...
                  <input value={form.name} onChange={e => set('name', e.target.value)} placeholder="اسم الموظف" style={inputStyle} required />
                </div>
                {!emp && (<>
                  <div>
                ...
                <div>
                  <label style={labelStyle}>رقم التأمين</label>
                  <input value={form.insurance_no} onChange={e => set('insurance_no', e.target.value)} placeholder="رقم التأمين الاجتماعي" style={inputStyle} />
                </div>
                <div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px', background: '#f8fafc', borderRadius: '10px', border: '1px solid #e2e8f0' }}>
                  ...
                  <label htmlFor="is_working" style={{ fontWeight: 700, fontSize: '14px', cursor: 'pointer', color: form.is_working ? '#10b981' : '#64748b' }}>
                    {form.is_working ? '✅ موظف نشط' : '⏸️ غير نشط'}
                  </label>
                </div>
              </div>
            </form>
            ...
            <div>
              <p style={{ color: '#64748b', fontSize: '14px', marginBottom: '16px', fontWeight: 600 }}>
                رفع أوراق الموظف (صورة الهوية، شهادات، عقد العمل، إلخ)
              </p>
        ...
        {activeTab === 'info' && (
          <div style={{ padding: '14px 20px', borderTop: '1px solid #e2e8f0', display: 'flex', justifyContent: 'flex-end', gap: '10px', background: '#fafbfc' }}>
            <button onClick={onClose} style={{ padding: '10px 18px', background: '#f1f5f9', border: 'none', borderRadius: '10px', fontWeight: 700, cursor: 'pointer', color: '#64748b' }}>إلغاء</button>
            <button form="emp-form" type="submit" disabled={saving} style={{ padding: '10px 24px', background: 'linear-gradient(135deg, #4f46e5, #06b6d4)', color: 'white', border: 'none', borderRadius: '10px', fontWeight: 800, cursor: saving ? 'not-allowed' : 'pointer', opacity: saving ? 0.7 : 1 }}>
              {saving ? 'جاري الحفظ...' : emp ? 'حفظ التعديلات' : 'إضافة الموظف'}
            </button>
          </div>
        )}
      </div>
...
const ContactsEmployees = () => {
  ...
  const handleDeleteDept = async (id) => {
    if (!window.confirm('هل أنت متأكد من حذف الإدارة؟')) return;
    try { await api.delete(`/departments/${id}`); toast.success('تم الحذف'); fetchAll(); }
    catch { toast.error('فشل الحذف'); }
  };

  const handleDeleteJt = async (id) => {
    if (!window.confirm('هل أنت متأكد من حذف الوظيفة؟')) return;
    try { await api.delete(`/job-titles/${id}`); toast.success('تم الحذف'); fetchAll(); }
    catch { toast.error('فشل الحذف'); }
  };

  const handleDeleteEmp = async (id) => {
    toast.error('لا يمكن حذف الموظفين من هنا — يُرجى مراجعة إعدادات النظام');
  };

  const TABS = [
    { id: 'departments', label: 'الإدارات', icon: <Building size={15} /> },
    { id: 'jobtitles', label: 'الوظائف', icon: <Briefcase size={15} /> },
    { id: 'employees', label: 'الموظفون', icon: <Users2 size={15} /> },
  ];
  ...
  return (
    <div style={{ padding: '32px', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '28px' }}>
        <div>
          <h1 style={{ fontSize: '28px', fontWeight: 900, color: '#1e293b', margin: 0, letterSpacing: '-0.02em' }}>الموظفون</h1>
          <p style={{ color: '#64748b', margin: '6px 0 0', fontSize: '14px', fontWeight: 600 }}>
            {users.length} موظف · {departments.length} إدارة · {jobTitles.length} وظيفة
          </p>
        </div>
        <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '16px' }}>
              {departments.length === 0 ? (
                <div style={{ gridColumn: '1/-1', padding: '60px', textAlign: 'center', color: '#94a3b8', background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0' }}>
                  ...
                  <p style={{ fontWeight: 700, margin: 0 }}>لا توجد إدارات بعد</p>
                </div>
              ) : departments.map(d => (
                <div key={d.id} style={{ background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', padding: '20px', boxShadow: '0 2px 8px rgba(0,0,0,0.04)', transition: 'transform 0.2s, box-shadow 0.2s' }}
                  ...
                  <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '14px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '6px' }}>
                      <button onClick={() => setDeptModal(d)} style={{ background: '#f1f5f9', border: 'none', borderRadius: '8px', padding: '5px 10px', cursor: 'pointer', fontSize: '11px', fontWeight: 700, color: '#475569' }}>تعديل</button>
                      <button onClick={() => handleDeleteDept(d.id)} style={{ background: '#fef2f2', border: 'none', borderRadius: '8px', padding: '5px 10px', cursor: 'pointer', fontSize: '11px', fontWeight: 700, color: '#dc2626' }}>حذف</button>
                    </div>
                  </div>
                  <h3 style={{ margin: '0 0 6px', fontSize: '16px', fontWeight: 800, color: '#1e293b' }}>{d.name}</h3>
                  <p style={{ margin: 0, fontSize: '13px', color: '#64748b', fontWeight: 600 }}>
                    المدير: <span style={{ color: '#4f46e5' }}>{d.manager_name || 'غير محدد'}</span>
                  </p>
                  <div style={{ marginTop: '12px', padding: '8px', background: '#f8fafc', borderRadius: '8px', fontSize: '12px', color: '#94a3b8', fontWeight: 600 }}>
                    {users.filter(u => u.department_id === d.id).length} موظف
                  </div>
                </div>
              ))}
            </div>
            ...
            <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 4px 6px rgba(0,0,0,0.04)' }}>
              {jobTitles.length === 0 ? (
                <div style={{ padding: '60px', textAlign: 'center', color: '#94a3b8' }}>
                  ...
                  <p style={{ fontWeight: 700, margin: 0 }}>لا توجد وظائف بعد</p>
                </div>
              ) : (
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                      <th style={{ padding: '14px 16px', textAlign: 'right', fontSize: '11px', fontWeight: 800, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.06em' }}>اسم الوظيفة</th>
                      <th style={{ padding: '14px 16px', textAlign: 'right', fontSize: '11px', fontWeight: 800, color: '#64748b', textTransform: 'uppercase' }}>عدد الموظفين</th>
                      <th style={{ padding: '14px 16px', textAlign: 'right', fontSize: '11px', fontWeight: 800, color: '#64748b', textTransform: 'uppercase' }}>إجراءات</th>
                    </tr>
                  </thead>
                  <tbody>
                    {jobTitles.map((jt, i) => (
                      <tr key={jt.id} style={{ borderBottom: '1px solid #f1f5f9', background: i % 2 === 0 ? 'white' : '#fafbfc' }}>
                        ...
                        <td style={{ padding: '14px 16px', color: '#64748b', fontSize: '13px' }}>
                          {users.filter(u => u.job_title_id === jt.id).length} موظف
                        </td>
                        <td style={{ padding: '14px 16px' }}>
                          <div style={{ display: 'flex', gap: '6px' }}>
                            <button onClick={() => setJtModal(jt)} style={{ background: '#f1f5f9', color: '#475569', border: 'none', borderRadius: '8px', padding: '5px 12px', cursor: 'pointer', fontSize: '12px', fontWeight: 700 }}>تعديل</button>
                            <button onClick={() => handleDeleteJt(jt.id)} style={{ background: '#fef2f2', color: '#dc2626', border: 'none', borderRadius: '8px', padding: '5px 12px', cursor: 'pointer', fontSize: '12px', fontWeight: 700 }}>حذف</button>
                          </div>
                        </td>
                      </tr>
                    ))}
          ...
          {activeTab === 'employees' && (
            <>
              <div style={{ position: 'relative', maxWidth: '380px', marginBottom: '16px' }}>
                ...
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="بحث بالاسم أو البريد..." style={{ ...inputStyle, paddingRight: '38px' }} />
              </div>
              <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 4px 6px rgba(0,0,0,0.04)' }}>
                {filteredUsers.length === 0 ? (
                  <div style={{ padding: '60px', textAlign: 'center', color: '#94a3b8' }}>
                    <Users2 size={40} style={{ opacity: 0.3, marginBottom: '12px' }} />
                    <p style={{ fontWeight: 700, margin: 0 }}>لا يوجد موظفون</p>
                  </div>
                ) : (
                  <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead>
                      <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                        {['الموظف', 'الوظيفة', 'الإدارة', 'الرقم القومي', 'تاريخ التعيين', 'الحالة', 'إجراءات'].map(h => (
                          <th key={h} style={{ padding: '14px 16px', textAlign: 'right', fontSize: '11px', fontWeight: 800, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.06em', whiteSpace: 'nowrap' }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {filteredUsers.map((u, i) => (
                        <tr key={u.id} style={{ borderBottom: '1px solid #f1f5f9', background: i % 2 === 0 ? 'white' : '#fafbfc', transition: 'background 0.15s' }}
                          ...
                          <td style={{ padding: '12px 16px' }}>
                            {u.is_working !== false ? (
                              <span style={{ background: '#dcfce7', color: '#16a34a', padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: 800 }}>✅ نشط</span>
                            ) : (
                              <span style={{ background: '#f1f5f9', color: '#64748b', padding: '3px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: 800 }}>⏸️ غير نشط</span>
                            )}
                          </td>
                          <td style={{ padding: '12px 16px' }}>
                            <button onClick={() => setEmpModal(u)} style={{ background: '#f1f5f9', color: '#475569', border: 'none', borderRadius: '8px', padding: '6px 12px', cursor: 'pointer', fontSize: '12px', fontWeight: 700 }}>تعديل</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
