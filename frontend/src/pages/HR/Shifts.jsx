...
const Shifts = () => {
  ...
  const fetchAssignments = async () => {
    ...
    try {
      ...
      toast.error('فشل تحميل تعيينات الموظفين');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSaveShift = async (e) => {
    e.preventDefault();
    if (!shiftForm.name.trim()) return toast.error('يرجى كتابة اسم الشيفت');
    try {
      if (editingShiftId) {
      ...
      toast.error(err.response?.data?.message || 'فشل حفظ الشيفت');
    }
  };

  // Delete Shift
  const handleDeleteShift = async (id) => {
    if (!window.confirm('هل أنت تأكد من حذف هذا الشيفت؟')) return;
    try {
  ...
  const handleSaveAssign = async (e) => {
    e.preventDefault();
    if (!assignForm.user_id || !assignForm.shift_id) return toast.error('يرجى اختيار الموظف والشيفت');
    try {
      await api.post('/hr/shifts/assign', assignForm);
      toast.success('تم تعيين الشيفت للموظف بنجاح');
      setShowAssignModal(false);
      fetchAssignments();
    } catch (err) {
      toast.error(err.response?.data?.message || 'فشل تعيين الشيفت');
    }
  };
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
        <div>
          <h2 style={{ margin: 0, fontSize: '22px', fontWeight: 800, color: '#1e293b', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <Clock size={24} style={{ color: '#06b6d4' }} /> شيفتات العمل وقواعد الخصم (Shifts Engine)
          </h2>
          <p style={{ margin: '4px 0 0 0', color: '#64748b', fontSize: '13px' }}>
            تعريف ساعات العمل والحضور والانصراف، أيام الإجازة، وقواعد الخصومات التراكمية
          </p>
        </div>
      ...
      <div style={{ display: 'flex', gap: '12px', borderBottom: '2px solid #e2e8f0', marginBottom: '24px' }}>
        ...
        >
          <Clock size={16} /> قائمة الشيفتات المعرفة ({shifts.length})
        </button>
        <button
        ...
        >
          <Users size={16} /> تعيين الموظفين للشيفتات
        </button>
      </div>
        ...
        <div>
          {loading ? (
            ...
            <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', padding: '60px', textAlign: 'center', color: '#94a3b8' }}>
              ...
              <p style={{ fontWeight: 700, margin: 0 }}>لا توجد شيفتات قائمة</p>
              <p style={{ fontSize: '13px', color: '#cbd5e1' }}>قم بإضافة الشيفت الصباحي أو المسائي لضبط حضور الموظفين</p>
            </div>
          ) : (
                ...
                <div key={s.id} style={{ background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', padding: '20px', boxShadow: '0 4px 12px rgba(0,0,0,0.03)', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                  <div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', background: '#f8fafc', padding: '12px', borderRadius: '10px', marginBottom: '14px', border: '1px solid #f1f5f9' }}>
                      <div>
                        <span style={{ fontSize: '11px', color: '#94a3b8', fontWeight: 700, display: 'block' }}>بداية ونهاية الشيفت</span>
                        <span style={{ fontSize: '14px', fontWeight: 800, color: '#0f172a' }}>{s.start_time} — {s.end_time}</span>
                      </div>
                      <div>
                        <span style={{ fontSize: '11px', color: '#94a3b8', fontWeight: 700, display: 'block' }}>سماح التأخير</span>
                        <span style={{ fontSize: '14px', fontWeight: 800, color: '#06b6d4' }}>{s.grace_minutes} دقيقة</span>
                      </div>
                    </div>

                    <div style={{ marginBottom: '14px' }}>
                      <span style={{ fontSize: '12px', fontWeight: 700, color: '#475569', display: 'block', marginBottom: '6px' }}>أيام الإجازة الأسبوعية:</span>
                    ...
                    <div>
                      <span style={{ fontSize: '12px', fontWeight: 700, color: '#475569', display: 'block', marginBottom: '6px' }}>قواعد الخصم التراكمي:</span>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                        {(s.deduction_rules || []).map((r, i) => (
                          <div key={i} style={{ fontSize: '12px', background: '#fffbeb', color: '#b45309', padding: '4px 10px', borderRadius: '6px', fontWeight: 700, display: 'flex', justifyContent: 'space-between' }}>
                            <span>تأخير من {r.from_min} إلى {r.to_min} دقيقة</span>
                            <span>خصم: {r.deduct_days} يوم</span>
                          </div>
                        ))}
                      </div>
                    </div>
      ...
      {activeTab === 'assignments' && (
        <div style={{ background: 'white', borderRadius: '16px', border: '1px solid #e2e8f0', overflow: 'hidden', boxShadow: '0 4px 15px rgba(0,0,0,0.03)' }}>
          {loading ? (
            ...
            <div style={{ padding: '60px', textAlign: 'center', color: '#94a3b8' }}>لا يوجد موظفون في النظام</div>
          ) : (
            <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'right' }}>
              <thead>
                <tr style={{ background: '#f8fafc', borderBottom: '2px solid #e2e8f0' }}>
                  ...
                  <th style={{ padding: '14px 18px', fontSize: '12px', fontWeight: 800, color: '#475569' }}>الشيفت المعين</th>
                  <th style={{ padding: '14px 18px', fontSize: '12px', fontWeight: 800, color: '#475569' }}>تاريخ السريان</th>
                ...
                {userSummary.map((u) => (
                  <tr key={u.user_id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                    ...
                    <td style={{ padding: '14px 18px', fontSize: '13px', color: '#64748b' }}>
                      {u.effective_from ? new Date(u.effective_from).toLocaleDateString('ar-EG') : '—'}
                    </td>
      ...
      {showShiftModal && (
        <div style={modalStyle}>
          <div style={{ background: 'white', borderRadius: '16px', width: '100%', maxWidth: '650px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}>
            <div style={{ padding: '18px 24px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#f8fafc', sticky: 'top' }}>
              <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 800, color: '#1e293b' }}>
                {editingShiftId ? 'تعديل بيانات الشيفت' : 'إنشاء شيفت جديد'}
              </h3>
            ...
            <form onSubmit={handleSaveShift} style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              <div>
                <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, color: '#475569', marginBottom: '6px' }}>اسم الشيفت</label>
                <input
                  type="text"
                  placeholder="مثال: الشفت الصباحي / شفت المقر الرئيسي"
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px' }}>
                <div>
                  <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, color: '#475569', marginBottom: '6px' }}>وقت البداية</label>
                  <input
                    type="time"
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, color: '#475569', marginBottom: '6px' }}>وقت النهاية</label>
                  <input
                    type="time"
                ...
                <div>
                  <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, color: '#475569', marginBottom: '6px' }}>دقائق السماح</label>
                  <input
                    type="number"
              ...
              {/* Off Days Checkboxes */}
              <div>
                <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, color: '#475569', marginBottom: '8px' }}>أيام الإجازة الأسبوعية (Off Days)</label>
              ...
              <div style={{ background: '#fafafa', padding: '16px', borderRadius: '12px', border: '1px solid #e2e8f0' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                  <label style={{ fontSize: '13px', fontWeight: 800, color: '#1e293b' }}>قواعد الخصم التراكمي في حالة التأخير</label>
                  <button
                    type="button"
              ...
              <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', marginTop: '12px' }}>
                ...
                >
                  {editingShiftId ? 'تحديث الشيفت' : 'حفظ الشيفت'}
                </button>
              </div>
            </form>
          </div>
        ...
        <div style={modalStyle}>
          <div style={{ background: 'white', borderRadius: '16px', width: '100%', maxWidth: '450px', overflow: 'hidden', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}>
            ...
            <form onSubmit={handleSaveAssign} style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div>
                <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, color: '#475569', marginBottom: '6px' }}>الشيفت الجديد</label>
                <select
              ...
              <div>
                <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, color: '#475569', marginBottom: '6px' }}>تاريخ السريان</label>
                <input
                  type="date"
