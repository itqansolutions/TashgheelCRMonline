...
exports.getStatuses = async (req, res) => {
    try {
        const result = await db.query(
            'SELECT * FROM lead_statuses WHERE tenant_id::text = $1::text ORDER BY sort_order ASC',
            [req.user.tenant_id]
        );
...
exports.updateStatus = async (req, res) => {
    ...
    try {
        const result = await db.query(
            'UPDATE lead_statuses SET name = $1, color = $2, sort_order = $3 WHERE id = $4 AND tenant_id::text = $5::text RETURNING *',
...
exports.deleteStatus = async (req, res) => {
    ...
    try {
        const result = await db.query(
            'DELETE FROM lead_statuses WHERE id = $1 AND tenant_id::text = $2::text AND is_default = false RETURNING *',
